...
public class Hangman_result_1p extends JFrame {

	private JPanel contentPane;
	private static Image backk=new ImageIcon(Hangman_result_1p.class.getResource("../image/mback.png")).getImage();
	ImageIcon ii = new ImageIcon(this.getClass().getResource("../image/runningman.gif"));
	static int back1X;
	static int back2X;
	static boolean mbackk;
	...
	class BackPanel extends JPanel{
			...
			new Thread(new Runnable() {
				...
				public void run() {
					while (mbackk) {
						back1X--;
						back2X--;
						if(back1X < -(backk.getWidth(null))) {
							back1X = backk.getWidth(null);
							back2X=back2X+1;
						}
						
						if(back2X < -(backk.getWidth(null))) {
							back2X = backk.getWidth(null);
							back1X=back1X+1;
						}
		...
		protected void paintComponent(Graphics g) {
			...
			g.drawImage(backk, back1X, 0, this);
			g.drawImage(backk, back2X, 0, this);
		}
	}
	...
	public Hangman_result_1p() {
        ...
        label_result.setText("총"+Hangman_main.cntStr+"번 만에 맞추셨습니다");
        Hangman_main.cnt2=0;
